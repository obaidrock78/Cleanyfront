...
function CompleteBooking({ open, handleClose, setFormData, formData, handleReset }) {
  const handleToggleChange = (value) => {
    const dupObj = { ...formData };
    dupObj.dispatch_now = value;
    setFormData(dupObj);
  };
  return (
    <>
      ...
      >
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', color: 'white', fontSize: '1.25rem' }}>
            ...
            >
              ...
              <Typography variant="h1">
                <span>It's time</span> to dispatch!
              </Typography>
            </Box>
          </Box>
          <CloseIcon
        ...
        >
          <p>This booking page is set to manual dispatching with a Team size of 1.</p>
          <Box display={'flex'} alignItems="center">
            <PeopleOutlinedIcon />
            <p>No servie provider has been dispatched for this booking.</p>
          </Box>
        </Box>
        <Box display={'flex'} alignItems="center" justifyContent={'space-between'}>
          <p>Do you want to dispatch now?</p>
          <Switch
            checked={formData?.dispatch_now}
