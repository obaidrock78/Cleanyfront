...
function RescheduleAppointment({ open, handleClose, bookindDetails }) {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      const dupObj = { ...values };
      setLoading(true);
      const Date = moment(values.time).format();
      dupObj.time = Date;
      toast.promise(
  ...
  useEffect(() => {
    ...
    setFieldValue('time', moment(bookindDetails?.appointment_date_time).format().slice(0, 16));
  }, [bookindDetails]);
